...
{
    public class ConsumerProductPriceMsg : ConsumerMessage<IEnumerable<ProductMsg>>
    {
        private readonly IProductReportService _productReportService;

        public ConsumerProductPriceMsg(IProductReportService productReportService)
           : base()
        {
            _productReportService = productReportService;
            InitAsPublishedMessage(AppConfiguration.Configuration["AppConfiguration:ReportProductsPriceQueueName"].ToString());
        }
        ...
        public override async Task ProcessMessage(Message<IEnumerable<ProductMsg>> messsage)
        {
            ...
            if (notification != null)
            {
                ...
                var productsDto = products.Select(x => new ProductDto(x.Id) { OldPrice = x.Price, NewPrice = x.NewPrice });
                await _productReportService.SendChangePriceNotifications(productsDto);
            }
        }
    }
}
